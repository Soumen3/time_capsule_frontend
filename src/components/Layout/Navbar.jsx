...
const Navbar = () => {
  ...
  const [loggingOut, setLoggingOut] = useState(false); // State for logout process
  ...
  return (
    <nav className="relative w-full max-w-7xl mx-auto mt-4">
      {/* Blur and disable overlay when logging out */}
      {loggingOut && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm pointer-events-auto">
          <div className="bg-white rounded-lg shadow-lg px-8 py-6 flex items-center space-x-3">
      ...
      {/* Blur the rest of the page when logging out */}
      <div className={loggingOut ? "pointer-events-none blur-sm select-none" : ""}>
